...
public class Inflection {
	...
	static {
		...
		// Singular ends in -us, plural ends in -i: alumnus/alumni, focus/foci,
		// nucleus/nuclei,
		// octopus/octopi, radius/radii, stimulus/stimuli, virus/viri
		// Exceptions to the above
		single2plurals.put("bus", "buses");

		// Singular ends in -ex, plural ends in -ices: appendix/appendices,
		// index/indices
		single2plurals.put("index", "indices");
		single2plurals.put("vertex", "vertices");

		// These include nouns that are traditionally plural, but are also used
		// for singular forms:
		...
		// Rule 7b: All musical terms ending in -o have plurals ending in just
		// -s.
	...
	public static String pluralize(String word) {
		...
		// Rule #5: For words that end in -is, change the -is to -es to make the
		// plural form
		else if (tmp.endsWith("is")) {
			plform = replaceLast(tmp, "is", "es");
		}
		// Singular ends in -ix, plural ends in -ices: appendix/appendices,
		// index/indices
		...
		// Rule #2: For words that end in a "hissing" sound (-s, -z, -x, -ch,
		// -sh), add an -es to form the plural.
		// Note: I removed tmp.endsWith("s") || as this cause
		// "posts"->"postses".
		else if (!tmp.endsWith("es") && (tmp.endsWith("z") || tmp.endsWith("x") || tmp.endsWith("ch") || tmp.endsWith("sh"))) {
			plform = tmp + "es";
		} else if (tmp.endsWith("y")) {
			// Rule #3: If the word ends in a vowel plus -y (-ay, -ey, -iy, -oy,
			// -uy), add an -s to the word.
			if (tmp.endsWith("ay") || tmp.endsWith("ey") || tmp.endsWith("iy") || tmp.endsWith("oy") || tmp.endsWith("uy")) {
				plform = word + "s";
			}
			// Rule #4: If the word ends in a consonant plus -y, change the -y
			// into -ie and add an -s to form the plural.
			else {
				plform = replaceLast(tmp, "y", "ies");
			}
		}
		// Rule #6: Some words that end in -f or -fe have plurals that end in
		// -ves.
		...
		// Rule #7: The plurals of words ending in -o are formed by either
		// adding -s or by adding -es
		else if (tmp.endsWith("o")) {
			// All words that end in a vowel plus -o (-ao, -eo, -io, -oo, -uo)
			// have plurals that end in just -s:
			if (tmp.endsWith("ao") || tmp.endsWith("eo") || tmp.endsWith("io") || tmp.endsWith("oo") || tmp.endsWith("uo")) {
				plform = word + "s";
			}
		...
		// Rule #8: The plurals of single capital letters, acronyms, and Arabic
		// numerals
	 ...
	 *            the original string
	 * @param oldSymbol
	 *            the old symbols to be replaced
	 * @param newSymbol
	...
	/**
	 * Converts string to Camel case. If <tt>firstLetterInLowerCase</tt> is
	 * true, then the first letter of the result string is in lower case.
	 *
	 * <pre>
	 * Examples:
	...
	/**
	 * Replaces all dashes and underscores by spaces and capitalizes all the
	 * words.
	 *
	 * <pre>
	 * Examples:
	...
	/**
	 * Returns a model class name corresponding to the input database table
	 * name.
	 *
	 * <pre>
	 * Examples:
